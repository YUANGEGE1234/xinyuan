...
def render_skill_page(
    ...
    return (
        ...
        "\n"
        ":::info\n"
        "The following is the complete skill definition that Xinyuan loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.\n"
        ":::\n"
        "\n"
        f"{body_clean}\n"
    )
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Xinyuan Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Xinyuan ships with a large built-in skill library copied into `~/.hermes/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Xinyuan also syncs bundled skills on `hermes update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.hermes/skills/` tree, it is still shipped with Xinyuan; restore it with `hermes skills reset <name> --restore`.",
        "",
