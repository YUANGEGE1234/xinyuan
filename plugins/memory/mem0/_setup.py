...
def _setup_platform(hermes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(f"\n  ⚠ MEM0_HOST is set in your environment ({os.environ['MEM0_HOST']}). It overrides platform mode — remove it from ~/.hermes/.env (or unset it) or Xinyuan will keep routing to the self-hosted server.")
