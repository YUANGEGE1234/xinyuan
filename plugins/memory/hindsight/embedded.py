...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    at import, so Xinyuan degrades instead of retrying a broken backend).
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    on the Xinyuan .env during post_setup, where a Notepad BOM would stick to the first key."""
    if not path.exists():
        return {}
