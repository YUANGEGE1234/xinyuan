...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Xinyuan ``/command`` text."""
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    typed_command_prefix = "!"  # clients reserve typed "/" for local commands; "!command" always reaches Xinyuan
        ...
        if self._access_token:
            ...
            try:
                resp = await client.login(
                    identifier=self._user_id, password=self._password, device_name="Xinyuan Agent",
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        words, or "Xinyuan Agent" would become "Agent"."""
        if not body:
            return ""
        if self._user_id:
...
def interactive_setup() -> None:
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        for line in ("📬 Home Room: where Xinyuan delivers cron job results and notifications.",
