"""langfuse — Xinyuan plugin tracing conversations, LLM calls and tool usage to Langfuse.
...
# Under a multiplexed profile override, one settled client (or _INIT_FAILED) per Xinyuan home: the
...
def _canonical_usage_and_cost(canonical: Any, *, provider: str, model: str,
    ...
    """Translate canonical Xinyuan usage into Langfuse usage and cost maps."""
    ...
    # need an explicit total (Xinyuan estimate also includes request pricing).
...
def _start_root_trace(task_key: str, *, task_id: str, session_id: str, platform: str, provider: str, model: str,
    ...
    def open_root():
        ctx = client.start_as_current_observation(trace_context=trace_ctx, name="Xinyuan turn", as_type="chain",
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(session_id=session_id or task_key, trace_name="Xinyuan turn",
...
def register(ctx) -> None:
    # Both hook-name variants so the plugin works across Xinyuan versions:
