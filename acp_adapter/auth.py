"""ACP auth helpers — detect and advertise Xinyuan authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Xinyuan runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Xinyuan.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Xinyuan credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Xinyuan using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Xinyuan provider", type="terminal", args=["--setup"],
        description=("Open Xinyuan' interactive model/provider setup in a terminal. "
                     "Use this when Xinyuan has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Xinyuan can resolve any runtime provider credentials."""
