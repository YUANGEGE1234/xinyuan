...
# Xinyuan tool name -> ACP ToolKind (anything unlisted is "other").
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    Some Xinyuan tools append a human hint after the payload (``{...}\\n\\n[Hint: ...]``),
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Xinyuan tool result clearly failed.
...
def build_tool_title(tool_name: str, args: Args) -> str:
    ...
    every other Xinyuan surface, so ACP clients never show a different summary than the CLI/TUI;
