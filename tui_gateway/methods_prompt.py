...
def _legacy_group_fence_error(rid, session, params):
    ...
    if hosted or peer:
        ...
        return _err(
            rid, 4122, f"This room is managed by {owner}. Update Xinyuan Desktop to continue it.")
    return None
...
def _parse_truncation_params(rid, sid, session, params, history):
    ...
    return None, None, _err(
        ...
        "(update your Xinyuan client if a rewind was intended)")
...
_PREVIEW_RESTART_RULES = (
    "Restart exactly the app intended for the Preview URL, not Xinyuan Desktop itself.",
    ...
    "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Xinyuan/Desktop process cwd.",
    ...
    "Before declaring success, verify the Preview URL responds with the intended app, not Xinyuan Desktop. If it serves Xinyuan/Desktop UI or another unrelated app, stop that process and report failure.",
