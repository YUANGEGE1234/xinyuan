...
<body>
    <div class="layout {layout_class}">
        ...
        <div class="main-content">
            ...
            <footer>
                Built with ☤ Xinyuan Agent • Generated on {generated_at}
            </footer>
        </div>
    </div>
    ...
    return f"""
        <div class="{"session-view" if is_multi else "session-view active"}" id="view-{escaped_sid}">
            <header class="fade-in">
                <h1>{_escape_html(s.get("title") or "Xinyuan Session")}</h1>
                <div class="meta">
...
def generate_multi_session_html_export(sessions: List[Dict[str, Any]]) -> str:
    ...
    return HTML_TEMPLATE.format(
        page_title="Xinyuan Session Export" if is_multi else _escape_html(sessions[0].get("title") or "Xinyuan Session"),
