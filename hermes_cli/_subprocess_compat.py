...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Xinyuan's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    Xinyuan shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_hermes(text: str) -> bool:
    r"""True when *text* names Xinyuan at a path-segment / token boundary.
...
def _process_command_is_hermes(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Xinyuan code."""
    try:
        import psutil
...
def pid_is_hermes(pid: int, *, expected_start_time: int | None = None) -> bool:
    ...
    The PID must be valid, currently exist, and identify a Xinyuan process. When the caller captured
