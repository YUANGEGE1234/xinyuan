...
``platform_gate_env``) resolves against — whether or not Xinyuan enumerates it anywhere. Roughly 290
...
def is_registered_env_name(name: str) -> bool:
    """True when Xinyuan itself enumerates ``name``: ``OPTIONAL_ENV_VARS`` / ``_EXTRA_ENV_KEYS``, or a
