...
# Highest manifest schema version this Xinyuan understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Xinyuan "
...
class PluginManifest:
    ...
    key: str = ""
    # Xinyuan version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    ...
    # VALIDATED AND SURFACED ONLY — Xinyuan never auto-installs these (isolation design for the install seam
