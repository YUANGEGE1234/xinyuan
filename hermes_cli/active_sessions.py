...
def active_session_limit_message(
    ...
    return (
        f"Xinyuan is at the active session limit ({active_count}/{max_sessions})."
...
def session_already_owned_message(session_id: str, entry: dict[str, Any]) -> str:
    ...
    return (
        "This chat is open in another Xinyuan window/terminal. Use it there, or start a new chat here.\n"
    ...
    with _FileLock(lock_path):
        ...
        if loaded is None:
            return None, ActiveSessionRefusal(
                "Xinyuan could not read the active-session registry at "
