"""Offline, non-destructive recovery for a damaged Xinyuan session database.
...
def _snapshot_and_inspect(
    ...
    try:
        ...
        if _source_fingerprint(source) != before:
            raise SessionRecoverySafetyError(
                "The source database bundle changed while it was being copied. Stop every Xinyuan process using this "
                ...
                "tracking) to state.db in the background and counts as a Xinyuan process even after the gateway is "
...
def _salvage_rowid_bounds(source: sqlite3.Connection, table: str) -> dict[str, Any]:
    ...
    # aggregate lets the planner answer from any covering index (every Xinyuan table has at
