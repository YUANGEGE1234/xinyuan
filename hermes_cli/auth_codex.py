...
Tokens live in ~/.hermes/auth.json, NOT ~/.codex/: Xinyuan keeps its own Codex OAuth session
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Xinyuan auth store (~/.hermes/auth.json)."""
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Xinyuan auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    """Refresh Codex OAuth tokens without mutating Xinyuan auth state."""
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    with _provider_state_transaction("openai-codex", lock_timeout) as (_store, state, _source):
        ...
        try:
            ...
            # (or another Xinyuan process) rotates the shared token this frozen copy fails with a
...
def resolve_codex_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    """Resolve runtime credentials from Xinyuan's own Codex token store.
    ...
    if _should_refresh(access_token):
        # Re-read under lock to avoid racing with other Xinyuan processes
...
def _login_openai_codex(args, pconfig: ProviderConfig, *, force_new_login: bool = False) -> None:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Xinyuan will create its own session to avoid conflicts with Codex CLI / VS Code.")
            if _prompt_yes_no(
                ...
                print("Xinyuan will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Xinyuan gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Xinyuan creates its own session — won't affect Codex CLI or VS Code)")
    print()
