...
Examples:
    ...
    hermes console                Open the safe Xinyuan command console
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Xinyuan Agent")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Xinyuan itself.")
...
def build_top_level_parser():
    ...
    parser = HermesArgumentParser(
        prog="hermes", description="Xinyuan Agent - AI assistant with tool-calling capabilities",
