...
def create_environment(env_type: str = "local", image: str = "python:3.11-slim", cwd: str = "/tmp", timeout: int = 60, **kwargs):
    """Create a Xinyuan execution environment (``local`` ignores ``image``/``kwargs``)."""
    if env_type == "local":
...
class MiniSWERunner:
    """Tool-calling agent loop over a Xinyuan execution environment, emitting Xinyuan trajectories."""
    ...
    def _convert_to_hermes_format(self, messages: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """Convert the OpenAI-style message list to the Xinyuan trajectory format used by batch_runner.py."""
...
def main(
    ...
    """
    Run SWE tasks with Xinyuan trajectory format output.
    ...
    """
    print("🚀 Mini-SWE Runner with Xinyuan Trajectory Format")
    print("=" * 60)
