...
DANGEROUS_PATTERNS = [
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hermes[^\n]*\.env\b', "access to Xinyuan secrets (Windows path)"),
    # ── end of Windows tier
    ...
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/Xinyuan file (key implant, login-time
    ...
    # coverage from #14639.
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Xinyuan config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/Xinyuan home prefixes to ~/ and ~/.hermes/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/HERMES_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). Xinyuan home
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Xinyuan ad-hoc temp script."""
    try:
