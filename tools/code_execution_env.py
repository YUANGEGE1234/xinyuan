...
# along or a child that imports Xinyuan code loses the Kanban mutation guard
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    if _uses_hermes_python_environment(child_python):
        ...
        logger.info("execute_code: child interpreter %s is outside the Xinyuan "
...
# Interpreter paths already reported as outside the Xinyuan environment.
...
def _uses_hermes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Xinyuan's active Python environment. Short-circuits when
