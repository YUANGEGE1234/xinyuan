...
def _http_cache_dir() -> Path:
    """Filesystem HTTP cache shared by every Lightpanda this Xinyuan spawns.
    ...
    bounds it without Xinyuan managing eviction.
    """
    ...
    if not binary:
        if os.name == "nt":
            return None, ("browser.engine is 'lightpanda' but Lightpanda has no Windows "
                          "build. Set browser.engine to auto (or run Xinyuan under WSL2).")
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Xinyuan is gone; return the count. A live owner is
