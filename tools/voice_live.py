"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to Xinyuan.
...
delegates every real request to Xinyuan as its *client-delegation* backend. Xinyuan stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — Xinyuan' answer is what the voice speaks.
...
# backend (Xinyuan) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are Xinyuan, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- Xinyuan agent: a full AI agent with tools — it can run commands, read and edit files, "
