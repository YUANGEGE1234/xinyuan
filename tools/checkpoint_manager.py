...
DEFAULT_EXCLUDES = [
    ...
    ".git/", ".hg/", ".svn/", ".worktrees/",  # VCS + worktrees (Xinyuan convention — don't snapshot siblings)
...
_STORE_GIT_CONFIG = (("user.email", "hermes@local"), ("user.name", "Xinyuan Checkpoint"),
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file``/``patch`` produced, so restores can tell Xinyuan' writes from later user edits."""
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Xinyuan just wrote (agent-write ledger), so safe-mode
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        """Classify files changed since ``commit_hash``: ``restore`` = still matching what Xinyuan
        last wrote (or deleted since); ``skipped`` = user-edited afterwards or never written by
        Xinyuan.  ``ledger_empty`` => no ledger, callers fall back to a full restore."""
    ...
    def restore(self, working_dir: str, commit_hash: str, file_path: str = None,
        ...
        the user hand-edited after Xinyuan' last write untouched (agent-write ledger); the result
