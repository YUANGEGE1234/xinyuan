...
# The cache is also persisted under the Xinyuan home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    active profile and is isolated across Xinyuan homes. The cache directory
    is created on demand. Returns ``None`` when Xinyuan home cannot be
