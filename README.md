...
| 3 | **品牌更名 Hermes → Xinyuan** | 覆盖 6 个语言的界面文案（共 1037 处）、`src/` 与 `electron/` 下的用户可见字符串（101 个文件、212 处）、`package.json` 的品牌字段、Windows 可执行文件的版本元数据、以及页面标题。**代码标识符一律未动**（`HERMES_*` 环境变量、`hermes://` 协议、`window.hermesDesktop` 等），保证功能不受影响。 |
