...
def _real_platform_state_root() -> Optional[Path]:
    """The REAL platform-default Xinyuan root. Avoids ``Path.home()`` /
...
# under Xinyuan test : isolation", and it inherits into subprocess children by default — so a : child that
