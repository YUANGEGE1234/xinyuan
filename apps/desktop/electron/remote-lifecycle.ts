...
async function locateHermes(ssh, remoteHermesPath) {
  ...
  const err: any = new Error(
    'Xinyuan is not installed on the remote host (could not find a `hermes` executable). ' +
      'Install it on the remote with:  curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Xinyuan path explicitly in the SSH connection settings.'
  )
...
// e.g. "Xinyuan Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH hermes a
...
async function probeRemoteHermesHome(ssh) {
  try {
    ...
    const error: any = new Error('Could not resolve the remote Xinyuan home.')
 ...
 * bytes; it never imports or executes code from the changing Xinyuan checkout.
...
async function assertRemoteInstallUpdateClear(ssh, hermesHome) {
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Xinyuan install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ...
      : 'The remote Xinyuan update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteHermesProfiles(ssh) {
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Xinyuan profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Xinyuan home.')
...
// execing Xinyuan. mutexPath is expandRemotePath() output — a complete shell
...
async function connect(deps) {
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Xinyuan Desktop build (${lock.reason}). ` +
