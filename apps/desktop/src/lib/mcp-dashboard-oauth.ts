...
const UPDATE_BACKEND = 'Update the Xinyuan backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Xinyuan Desktop to support MCP OAuth callbacks.')
  }
