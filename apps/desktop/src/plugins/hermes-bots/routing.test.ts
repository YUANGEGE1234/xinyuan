 ...
 * the raw Cloud hostname, or as generic "Xinyuan" when Cloud was the only
 * source.
 *
...
describe('alias identity survives the hosted handoff (#89131)', () => {
  ...
  it('renders the sole Cloud-only default as the alias, not "Xinyuan"', () => {
  ...
  it('never leaks the alias onto a same-named default on another connection', () => {
    ...
    // Local default while the ACTIVE gateway is local: untouched "Xinyuan".
    expect(displayName({ name: 'default' }, null)).toBe('Xinyuan')
  })
