...
export const host = {
  ...
  connections: async () => {
    ...
    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Xinyuan Desktop.')
    }
  ...
  agents: async () => {
    ...
    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Xinyuan Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Xinyuan Desktop to open another Bot chat.' })
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Xinyuan Desktop connection routing unavailable')
    }
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Xinyuan gateway unavailable')
    }
