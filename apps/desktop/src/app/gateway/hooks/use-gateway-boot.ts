    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Xinyuan backend'
        )
    ...
    configureGatewayRegistry({
      onServerRequest: request => {
        if (!callbacksRef.current.handleServerRequest(request)) {
          request.fail(JSON_RPC_METHOD_NOT_FOUND, `Xinyuan Desktop cannot answer ${request.method}`)
        }
      },
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen())) {
          throw new Error('Xinyuan gateway is not connected')
        }
    ...
    async function boot() {
      ...
      try {
        ...
        const conn = await withTimeout(
          desktop.getConnection(windowProfileOverride() ?? undefined),
          BACKEND_BOOT_WAIT_TIMEOUT_MS,
          'Timed out connecting to Xinyuan backend'
        )
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "Xinyuan couldn't start"
