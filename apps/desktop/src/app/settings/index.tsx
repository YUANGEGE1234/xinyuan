...
import { XinyuanSettings } from './xinyuan-settings'
...
const SETTINGS_VIEWS: readonly SettingsViewId[] = [
  ...
  'connections',
  'keybinds',
  // Legacy alias: the API Keys page is gone. Kept in the enum so a saved
  // `?tab=keys` deep link resolves and can be redirected below — dropping it
  // would make `useRouteEnumParam` coerce the tab to the default instead.
  'keys',
  'vault',
  'notifications',
  'billing',
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  // The separate API Keys page is gone — its only job is now the single
  // Xinyuan form. Redirect stale bookmarks / deep links rather than falling
  // through to the last branch (which would render the sessions pane).
  useEffect(() => {
    if (activeView === 'keys') {
  ...
  const exportConfig = async () => {
    try {
      ...
      a.href = url
      a.download = 'xinyuan-config.json'
      a.click()
  ...
  const navGroups: OverlayNavGroup[] = useMemo(
    () => [
      ...
      // The entire provider surface collapsed into one entry: this build talks
      // to a single fixed endpoint, so there is no catalog to browse.
      {
  ...
  const activeSettingsContent =
    activeView === 'config:appearance' ? (
      ...
      <XinyuanSettings onConfigSaved={onConfigSaved} />
