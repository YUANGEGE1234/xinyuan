...
export interface DesktopOnboardingState {
   ...
   *  overlay show its "Xinyuan is ready" screen once even though the app is
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Xinyuan is ready', message: `${provider} connected.` })
}
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'Xinyuan could not save the selected model.')
...
export async function refreshOnboarding(ctx: OnboardingContext, stillWanted?: () => boolean) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Xinyuan Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Xinyuan still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
