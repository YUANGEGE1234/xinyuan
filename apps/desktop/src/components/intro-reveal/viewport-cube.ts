...
function textureImage(): HTMLImageElement | null {
  ...
  // Light-on-dark line work: the faces keep their shading and the RGB channel
  // split has edges to offset. (A dark-on-white tile reads as a solid white
  // block once it is wrapped around a cube, which is why the BrandMark art was
  // never used here.) `intro-mark.png` carries no brand marks.
  img.src = `${import.meta.env.BASE_URL}intro-mark.png`
