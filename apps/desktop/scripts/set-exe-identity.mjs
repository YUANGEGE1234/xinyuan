#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Xinyuan icon + version metadata onto the
// built Xinyuan.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Xinyuan.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-Xinyuan.exe>
//
...
// Stamp the Xinyuan icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Xinyuan',
      FileDescription: 'Xinyuan',
      CompanyName: 'Xinyuan',
      LegalCopyright: 'Copyright (c) 2026 Xinyuan'
    }
  })

  console.log('[set-exe-identity] done — Xinyuan icon + identity stamped')
}
