...
Owns one Codex thread per Xinyuan session: drives ``turn/start``, consumes
...
# Xinyuan' tools.terminal.security_mode -> Codex permissions profile id.
...
class CodexAppServerSession:
    """One Codex thread per Xinyuan session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(client_name="hermes", client_title="Xinyuan Agent", client_version=_get_hermes_version())
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via Xinyuan' approval flow.
...
# Xinyuan approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a Xinyuan approval choice onto codex's approval decision wire value."""
...
def _get_hermes_version() -> str:
    """Best-effort Xinyuan version string for codex's userAgent line."""
    try:
