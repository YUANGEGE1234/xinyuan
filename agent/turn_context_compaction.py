...
def _codex_native_auto_compaction(agent: Any) -> bool:
    """Codex app-server threads are compacted by the codex agent itself; Xinyuan only
...
def _preflight_compression(
    ...
    if _preflight_deferred:
        ...
        logger.info(
            "Skipping Xinyuan preflight compression for codex app-server "
            "(mode=%s); Xinyuan will not start thread compaction here.",
