"""OpenAI-compatible shim that forwards Xinyuan requests to `copilot --acp`.
...
_PROMPT_PREAMBLE = (
    "You are being used as the active ACP agent backend for Xinyuan.",
...
_DEPRECATED_CLI_ERROR = (
    "Xinyuan ACP mode requires the NEW GitHub Copilot CLI (github.com/github/copilot-cli), but the binary it just "
    "spawned is the deprecated `gh copilot` extension.\n\n"
    "Install the new CLI:\n  npm install -g @github/copilot\n  # then verify with: copilot --help\n\n"
    "If `copilot` already resolves to the new CLI but you still see this,\npoint Xinyuan at it explicitly:\n"
...
def _format_messages_as_prompt(
    ...
    # one. Copilot has no tools of its own that collide with Xinyuan', so forward the whole toolset.
