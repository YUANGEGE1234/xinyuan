...
``skill_manage`` per the Xinyuan authoring standards; large prose sources get the knowledge-base layout (lean SKILL.md
...
Follow the Xinyuan skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Xinyuan`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Xinyuan.
...
4. "## How to Run" — the canonical invocation, framed through Xinyuan tools.
