"""OpenAI-shape bridge shared by Xinyuan' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Xinyuan' tool schemas travel INTO the
...
read/edit/execute tools forwards only Xinyuan' agent-level tools, since re-offering overlapping ones
makes Xinyuan redo finished work.
"""
...
class StreamChunks(list):
    ...
    Xinyuan reads off the ``create`` result; a plain list would drop them on the stream path."""
