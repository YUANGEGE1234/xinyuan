"""Core NeMo Relay adapters for physical Xinyuan provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Xinyuan knows the API mode."""
...
class _ManagedAttempt:
    ...
    def run_callback(self, callback: Callable[..., Any], *args: Any) -> Any:
        """Run a Xinyuan callback in a fresh copy of the captured context.
...
def _current_session_id() -> str | None:
    """Return the inherited Xinyuan turn's session id, or None outside a live turn."""
...
def execute(
    ...
    ``session_id`` defaults to the inherited Xinyuan turn's session (unmanaged when there is none)."""
...
# Run under the inherited Xinyuan turn when present (callers that do not know a session id).
...
def stream_current(
    ...
    """Run a provider stream under the inherited Xinyuan turn when present.
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Xinyuan."""
...
def _complete_logical(
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning("Xinyuan Relay logical LLM finalization failed", exc_info=True)
            return
        with turn.logical_llm_lock:
