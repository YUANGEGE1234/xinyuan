...
def _hermes_root_path() -> Path:
    """Xinyuan root dir (parent of any profile, never per-profile)."""
...
_READ_DENIED_DIRS = (
    ("mcp-tokens",
     "is the Xinyuan MCP token directory and cannot be read directly.",
     "is a Xinyuan MCP token file and cannot be read directly."),
    ("browser-profile",
     "is the Xinyuan real-profile browser snapshot directory (copied cookies/logins) and cannot be read directly.",
     "is inside the Xinyuan real-profile browser snapshot (copied cookies/logins) and cannot be read directly."),
    # vault.key + vault.json.enc sit side by side; key + ciphertext = plaintext, so the whole dir is one credential.
    ("vault",
     "is the Xinyuan credential vault directory and cannot be read directly (secrets are filled server-side by browser_vault_fill).",
     "is inside the Xinyuan credential vault (encrypted secrets + local key) and cannot be read directly (browser_vault_fill resolves them server-side)."),
)


def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Xinyuan path.
    ...
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in hermes_dirs):
        reason = (
            "is an internal Xinyuan cache file and cannot be read directly to prevent "
        ...
        reason = (
            "is a Xinyuan credential store and cannot be read directly. Provider tools "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Xinyuan read (see ``get_read_block_error``).
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Xinyuan state: ``None``
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host Xinyuan process never reads — the "
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host Xinyuan "
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Xinyuan scope, or is inside
