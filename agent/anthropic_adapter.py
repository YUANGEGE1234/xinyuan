"""Anthropic Messages API adapter: client construction + the Messages call for Xinyuan's
...
# Xinyuan effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain Xinyuan tool schemas/prose as a
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    Xinyuan tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning Xinyuan shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # which silently hides reasoning text that Xinyuan surfaces in its CLI. We request "summarized" so the
